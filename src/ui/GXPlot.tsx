...
export default function GXPlot(props: { model: ModelAPI; T: number; eq: {xA:number;xB:number}|null; showDerivs:{g1:boolean;g2:boolean} }) {
  const { model, T, eq, showDerivs } = props;
  ...
  const tangent = useMemo(() => {
    if (!eq) return null;
    const { m, b } = tangentLine(model, 'liq', eq.xA, T);
    return { m, b };
  }, [eq, model, T]);

  const tangentY = xs.map(x => tangent ? tangent.m * x + tangent.b : NaN);
  ...
  if (eq && tangent) {
    data.push({ x: xs, y: tangentY, name: 'tangent', type:'scatter', mode:'lines', line:{ color:'#444', dash:'solid', width:1 } });
    data.push({ x: [eq.xA, eq.xB], y: [model.g('liq', eq.xA, T), model.g('sol', eq.xB, T)], name: 'tie-line', type:'scatter', mode:'lines', line:{ color:'#777', dash:'dash' }, showlegend:false });
    data.push({ x: [eq.xA], y: [model.g('liq', eq.xA, T)], name: 'x_liq', type:'scatter', mode:'markers', marker:{ color:'#1f77b4', size:8 } });
    data.push({ x: [eq.xB], y: [model.g('sol', eq.xB, T)], name: 'x_sol', type:'scatter', mode:'markers', marker:{ color:'#d62728', size:8 } });
  }
